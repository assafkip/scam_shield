...
void main() {
  ...
  group('Youth Scenario Flow', () {
    ...
    setUpAll(() async {
      ...
      youthScenarios = allScenarios
          .where((s) => s.id.startsWith('y0'))
          .toList();
    });

    test('y01_dating scenario plays through golden path', () async {
      final datingScenario = youthScenarios.firstWhere(
        (s) => s.id == 'y01_dating',
      );
      ...
      final chosenOption = engine.makeChoice(
        'suggest_video',
      ); // "Suggest a quick video call instead" (safe)
